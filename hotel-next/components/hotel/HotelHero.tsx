...
const proofChips = [
  ...
  { label: "Rapid deployment", warm: true },
  { label: "Fully on-prem" },
];
...
export function HotelHero() {
  return (
    ...
    >
      {/* Backgrounds clip here only — content flows naturally so nothing gets cut off on resize */}
      <div
      ...
      {/* Decorative framing — non-essential; stays inside padded area */}
      <div
        className="pointer-events-none absolute left-6 right-6 top-6 hidden h-px bg-[rgba(21,33,47,0.88)] md:left-[4.5rem] md:right-[4.5rem] md:top-24 md:block xl:left-[72px] xl:right-[72px]"
        aria-hidden
      />
      <div
        className="pointer-events-none absolute bottom-6 left-6 right-6 hidden h-px bg-[rgba(21,33,47,0.88)] md:bottom-[5.75rem] md:left-[4.5rem] md:right-[4.5rem] md:block xl:left-[72px] xl:right-[72px]"
        aria-hidden
      />
      <div
        className="pointer-events-none absolute left-6 top-6 hidden h-0.5 w-16 rounded-full bg-memo-amber opacity-[0.32] md:block md:left-[4.5rem] md:top-24 xl:left-[72px]"
        aria-hidden
      />
      <div
      ...
      >
        <div className="flex w-full min-w-0 flex-col items-center">
          ...
          <h1
            id="hero-title"
            className="m-0 w-full min-w-0 max-w-[min(100%,36rem)] text-balance sm:max-w-[40rem] md:max-w-[44rem] lg:max-w-[52rem]"
          >
            <span className="block text-[clamp(1.875rem,calc(1.25rem+3.8vw),4.125rem)] font-semibold leading-[1.05] tracking-[-0.045em] sm:leading-[1.02]">
              Radio traffic is where hotel service issues surface first.
            </span>
            <span className="mt-3 block text-pretty text-[clamp(1.0625rem,calc(0.85rem+1.35vw),2.125rem)] font-bold leading-snug tracking-[-0.02em] text-memo-orange sm:mt-4 sm:leading-tight">
